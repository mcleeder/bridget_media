...
radio/             ← Radio France live-metadata client (peer of player/, HTTP only)
...
- **`display/state_machine.py`** — `AppState` enum (`HOME`, `PODCAST_LIST`, `EPISODE_LIST`, `NOW_PLAYING`, `QUEUE`, `BLUETOOTH`, `BLUETOOTH_DISCOVER`, `RADIO_LIST`, `RADIO_PLAYING`, `WIFI`, `WIFI_SETUP`, `SLEEP_TIMER`) and a pure `transition(state, event, context) → AppState`. No side effects live here. NOW_PLAYING and SLEEP_TIMER are each reachable from more than one screen, so Back from them returns to the matching origin in the frozen `NavigationContext(now_playing_origin, sleep_timer_origin)` — `ScreenManager` tracks both and passes them in, keeping `transition()` pure.
- **`display/events.py`** — frozen event dataclasses: `HomeMenuSelected(item: HomeMenuItem)`, `FeedSelected`, `EpisodeSelected`, `BackRequested`, `ListScrolled`, `PlayPauseToggled`, `SkipRequested`, `QueueToggled(episode)`, `QueueRemoveRequested(episode)`, `BluetoothDeviceSelected(device)`, `BluetoothScanRequested`, `BluetoothPairRequested(device)`, `BluetoothForgetRequested(device)`, `StationSelected(station)`, `SleepTimerRequested()`, `SleepDurationSelected(minutes: int | None)`.
...
| NOW_PLAYING / RADIO_PLAYING | SleepTimerRequested | SLEEP_TIMER |
| SLEEP_TIMER | SleepDurationSelected / BackRequested | `sleep_timer_origin` |
...
9. **Radio playing** — **two body layouts.** When Radio France says what is on air: the station's full French name small at the top, the track title in « guillemets » as the headline, artist under it, `Album · Année`, and an `ICON_GRAPHIC_EQ` + "En direct" marker anchored above the controls. When it does not — a jingle, a talk break, or the metadata call failing — the original layout takes over: station name centred, `ICON_GRAPHIC_EQ` + "En direct" under it. Either way a two-button control bar: Back keeps Now Playing's position and width so the gesture is identical on both players, and play/pause takes the whole remainder (there is no seeking on a live stream, so the ±30s buttons have nothing to do). The player-unreachable notice replaces the "Live" line
10. **Sleep timer** — a 2x2 grid of durations (15 / 30 / 45 / 60), not a list screen. Tapping a cell arms that duration and returns to the player; tapping the armed (inverted) cell clears it, so no separate Off control has to earn space. Header is the back button. Reached only from the two players, via the badge described below
11. **Wi-Fi** — read-only status, no list: network name with a state line under it (Connected / No internet / Setup hotspot), IP address, and `bridget.local`. Header is the back button and the body has no tap targets. Checking / offline / no-address / unreachable states; unreachable is always what Windows shows
...
├── radio/
│   ├── __init__.py
│   └── metadata.py            # Radio France livemeta client: TrackMetadata, RadioMetadataError
├── display/
│   ├── __init__.py
...
│   ├── sleep_timer.py         # SleepTimer + pure remaining/expired (injected clock)
...
│   ├── radio_metadata.py      # TrackMetadata / RadioMetadataService Protocols (read-only)
...
│       ├── sleep_badge.py       # the moon control shared by both players
│       ├── sleep_timer.py       # duration grid
│       └── wifi_list.py         # read-only network status
├── assets/
│   ├── fonts/                 # DejaVuSans.ttf, MaterialIcons-Regular.ttf (+ .codepoints)
│   └── wordlist.txt           # EFF Short Wordlist #1 — the hotspot password's words
...
db                        (knows config, knows nothing else)
    ↓
feeds / player / bluetooth / network / radio (know db, know config — bluetooth, network and radio need neither; they shell out or call HTTP)
    ↓
...
**Track titles — done 2026-08-23, see Phase 11.5.** The note that stood here said this was left out deliberately because it would add a polled external HTTP dependency to a device meant to run unattended. That objection is answered by bounding *when* it polls rather than by not doing it: requests happen only while the radio player is the screen on display, so a box sitting on Home, or playing a podcast, makes none at all.
...
- [x] `network/hotspot.py` — SSID (`Bridget-Setup-<4 chars>`) and password generated **once per device with `secrets`**, never a repo constant, and written `0600` via `os.open(O_EXCL)` so the password is never briefly world-readable. The password format changed on 2026-08-23 from twelve random characters to `word_word_dddd` — see the amendment below
...
**Amended 2026-08-23 — the hotspot password is words, not characters.** Twelve random characters (`k7m9qp2rxw4n`) is miserable to transcribe off an e-ink panel: every character has to be read individually, and there is no way to tell you have mistyped one until the join fails with no explanation. It is now two words and six digits — `gummy_music_390998` — from `assets/wordlist.txt` (EFF Short Wordlist #1, CC BY 3.0 US, every word 3–5 letters and free of confusable or offensive entries; `yo-yo` dropped, since a hyphen is an awkward keystroke beside the underscores). `_load_words` rejects a list that is short, duplicated or not pure lowercase, so a truncated file fails loudly instead of quietly shrinking the keyspace.

**The entropy cost is real and was priced, not hand-waved: ~41 bits against ~60.** It shipped at four digits (~34 bits) and went to six the same day, which is where the number was bought back — digits are the right place to spend length, because a contiguous numeric run is the cheapest thing on the panel to read and to type, so two more of them cost far less than another word or a wider alphabet would. What makes ~41 bits acceptable *here* is the shape of the attack rather than the number. To use it, someone must be in radio range during the specific window an AP is up — hard-capped at 20 minutes — capture both the handshake and the portal traffic, and only then grind offline; and the per-device SSID is the PBKDF2 salt, so nothing precomputed helps. Underscores also need no `WIFI:` escaping, so the QR payload got simpler rather than more fragile.

Verified by a 24-check harness that reads `_PASSWORD_WORDS` / `_PASSWORD_DIGITS` rather than hard-coding them, so changing either re-verifies rather than silently passing: list validation (short / uppercase / hyphenated / duplicated / missing / comment-only), format and WPA2's 8–63 bounds over 4000 draws, distinct words, digit coverage in every position, the widest possible password not clipping on the panel, the QR still rendering, and the ensure/load round trip. Six digits push some payloads from QR version 3 to 4, but both sit at the same **2px per module** in the 105px zone — an 82px symbol against 74px — so scannability is unchanged.
...
### Phase 11 — Sleep Timer ✅ COMPLETE (2026-08-23 — simulator-verified; confirm on Pi)
...
Shipped essentially as planned, with two deliberate departures noted below.

- [x] `display/sleep_timer.py` — frozen `SleepTimer(deadline)` plus pure `start`, `remaining_seconds`, `is_expired`, `remaining_minutes`, all taking `now` **as an argument**; `DURATION_CHOICES = (15, 30, 45, 60)` lives here too, so the grid and the manager cannot disagree about what is on offer
- [x] **`time.monotonic()`, never wall clock** — the Pi 3B+ has no RTC (see Key Notes), so a wall-clock deadline would fire hours early or never, and both failures happen while the owner is asleep. A deliberate divergence from the Phase 10 watchdog, which *had* to use wall clock because it exits between invocations
- [x] **Session state, never persisted** — a timer that survived a restart would stop playback at a deadline nobody set
- [x] **An absolute deadline, not "N minutes of playback"** — pausing does not pause the timer
- [x] Expiry check at the top of `ScreenManager.refresh_playback()`, **unconditionally** — not inside the `_playing_episode` poll (radio never enters it) and not inside the player-screen redraw (a timer set then left on Home must still fire)
- [x] **Expiry does not look like a natural finish.** `_check_sleep_timer` routes through `_release_playing_episode()` *before* stopping the player, so `_is_natural_finish` never sees a stop-after-playing-near-the-end and `_advance_queue` cannot start the next queued episode. Structurally immune, exactly as user-Back already is — no new flag, no special case
- [x] Podcast expiry persists the play position and does **not** mark the episode played; radio expiry clears `_playing_station`
- [x] The screen is not navigated away from on expiry — e-ink holds its last frame, so a stopped player is fine to wake up to
- [x] **`display/screens/sleep_badge.py`** — the control and the readout in one glyph, top-right of both players: a bare `ICON_BEDTIME` (`ef44`, verified against the codepoints file) when unset, an **inverted pill** with the whole minutes left when armed. Inverted because on 1-bit ink "a timer is running" has to read at a glance. The pill is a **fixed 44px** whatever it holds, so arming or clearing never reflows the feed name beside it
- [x] **The tap target is deliberately larger than the ink** — `TOUCH_LEFT = 224`, `TOUCH_BOTTOM = 30`, a 72x30 zone reaching down over the title, which is not a tap target. That is what answers the plan's flagged risk (a ~22px control against 35px list rows) without moving any existing layout. Now Playing's feed name shares the row, so it clips at `sleep_badge.INK_LEFT`, the same idiom as list rows clipping at `ACTION_X`; Radio Playing's top strip was already free
- [x] Both players pull the value through an injected `Callable[[], int | None]` rather than being handed a number at construction — `refresh_playback` redraws them every tick, so the countdown can never go stale
- [x] `AppState.SLEEP_TIMER`; `SleepTimerRequested` and `SleepDurationSelected(minutes: int | None)`; frozen `NavigationContext(now_playing_origin, sleep_timer_origin)` keeps `transition()` pure and three-argument

**Departure 1 — a 2x2 grid, not a scrolling list.** The plan said reuse `list_layout` with 15/30/45/60/Off as five rows against three visible. Rendered, that puts **Off** below the fold, and Off is exactly what you reach for when you set the wrong thing, half asleep in the dark. The grid makes every choice one tap on a 148x52 target — four times a list row's area. Cost is ~40 lines of layout no other screen reuses.

**Departure 2 — no Off control at all.** Cancelling is tapping the armed cell again. The active cell is drawn inverted, so the affordance is already on screen and a fifth control would only add something to miss. `ICON_BEDTIME_OFF` was therefore never needed.

**Bug found by the harness, not by review:** origins were recorded as "the next state happens to be NOW_PLAYING". Returning from the duration screen *is* such a transition, so it recorded `SLEEP_TIMER` as the origin and Back then bounced between the two screens with no way out to the episode list. Origins are now recorded from the event that **enters** a screen (`EpisodeSelected`, `SleepTimerRequested`) — see the Key Note.

- Verified by a 47-check harness: pure clock arithmetic with an injected `now` (rounding up so a fresh 30 reads "30m" and the last partial minute reads "1m"), then scripted touches for set-from-both-players, tap-to-clear, Back returning to the right player, podcast expiry persisting position without marking played, **expiry at 96% stopping instead of auto-advancing a two-entry queue**, radio expiry releasing the station, and the timer firing while sitting on Home. Six rendered states reviewed; `touch_zones.png` regenerated; `ruff` + `mypy --strict` clean; simulator smoke run clean
...
### Phase 11.5 — Live Track Metadata + French Naming ✅ COMPLETE (2026-08-23 — simulator-verified; confirm on Pi)

- [x] `radio/metadata.py` — a peer of `player/`, knowing only `config`. `RadioMetadataClient.get_current_track(station_id) -> TrackMetadata | None`, frozen `TrackMetadata(title, artist, album, year)`, `RadioMetadataError`. Uses `urllib` rather than `requests`, so the player app's runtime dependencies are unchanged. `display/radio_metadata.py` mirrors it as Protocols, so `display` still imports nothing below it
- [x] **The current track comes from the server's own cursor, not from timestamps.** The payload carries `levels[0].position` indexing `levels[0].items`; each step also has `start`/`end` unix times, and comparing those against our clock is the obvious approach and the wrong one — the Pi 3B+ has no RTC, so after an offline boot every window comparison picks the wrong track or none. The cursor needs no clock of ours
- [x] **Artist comes from `highlightedArtists`, not `authors`.** `authors` lists every session player: one observed jazz value named five people and ran past 90 characters, which is three wrapped lines on a 296px panel. `highlightedArtists` is Radio France's own answer to whose record it is; `authors` remains the fallback
- [x] Non-song steps (`embedType != "song"` — jingles, talk breaks) return `None` rather than a bad guess, and every field beyond the title is optional because the feed is inconsistent about albums, years and even artists
- [x] **Station ids confirmed one at a time against what each was actually playing**, not assumed: FIP 7, Rock 64, Jazz 65, Groove 66, Monde 69, Nouveautés 70, Reggae 71, Électro 74 — each verified by genre match (Leroy Smart on 71, a 1958 west-coast vocal session on 65, and so on)
- [x] Fetched on the Phase 8 background-thread terms (HTTP only, no sqlite, never draws; result crosses back through a `queue.Queue` drained by `poll_background_work()`), with a `_track_generation` counter so a late result can never label one station's stream with another's track. `_abandon_track_fetch()` runs on station change, on Back out of the player, on starting an episode, and on sleep-timer expiry
- [x] **Polling is bounded to "the radio player is on screen"** (`RADIO_METADATA_POLL_SEC = 20`), which is what makes an external HTTP dependency acceptable here at all — an unattended box on Home makes zero requests. A redraw only happens when the *track* changed, not on every poll: a track lasts minutes, so redrawing every 20s would flash the panel for nothing. **20s is a conservative default, not a limit** — measured on the device the call is 4.3KB and 0.22s mean (0.30s worst of five), so polling far more often would cost little; it was set high because a track lasts minutes and the redraw is already gated on the track actually changing, which is what caps the panel refreshes
- [x] `Station` gains `full_name` and `metadata_id`. **`name` stays short for the list, `full_name` is used on the player** — every full name begins with the same three words, so a list of them would be unscannable, while the player has one name on screen and room to say it properly
- [x] French where it is the station's own language: **« guillemets »** around the track title (real French typography, and it separates title from artist without a font weight the 1-bit canvas does not have), **"En direct"** replacing "Live", and `Album · Année` with a middle dot. All in `display/copy.py`; DejaVu Sans carries every glyph
- Verified by a 16-check harness — parser hardening against empty/short/out-of-range payloads, curated-artist preference and its fallback, title-only and blank-title steps, then a **live call to all eight stations** (all eight returned a named track) — plus seven rendered frames reviewed (full metadata, long title and album, title-only, no metadata, player unreachable, and a track coexisting with an armed sleep badge). The Phase 10/11 harnesses were re-run green; `ruff` + `mypy --strict` clean; simulator smoke run clean
...
- **A password a stranger has to retype is a UI, and random characters are a bad one.** The setup hotspot's twelve-character password was chosen for entropy alone, and it is read off a 296×128 e-ink panel and typed into a phone by someone who did not pick it and cannot check their work: every character needs an individual glance, a transposition is invisible, and the only feedback is a join that fails silently. Words are chunked and self-checking — you can hold `awoke_mummy` in your head for the length of a glance away. Going from ~60 to ~41 bits is a genuine cost, and the right move was to price it against the actual attack (in radio range, during a ≤20-minute window, capturing the handshake *and* the traffic, against a per-device salt) rather than to defend the bigger number reflexively. Digits carry the tail of the entropy because a contiguous numeric run is the cheapest thing to type — which is also why the fix for a format that felt too weak at ~34 bits was two more digits rather than a third word.
- **When an external dependency is the only way to get something, bound *when* it runs rather than refusing it.** Live track titles sat in the "deliberately not done" pile for months because polling a third-party HTTP API on an unattended box felt wrong. The fix was not a better API — it was noticing that the data is only ever *looked at* on one screen, so the poll can be gated on that screen being visible. A box on Home, or playing a podcast, now makes zero requests, and the objection evaporates without giving up the feature. Ask what the smallest window is in which the data actually matters before deciding a dependency is unaffordable.
- **A "current item" cursor from the server beats timestamps you have to evaluate against your own clock.** Radio France's payload offers both: `start`/`end` unix times per step, and a `levels[0].position` index. The timestamps are the more obvious tool and would have been wrong on this hardware in exactly the situation that matters — a Pi with no RTC, freshly booted and not yet NTP-synced, which is precisely when someone turns the box on. Prefer the answer that does not depend on the local clock being right; when both are offered, that choice is free.
- **Record a navigation origin from the event that *enters* a screen, never from "the next state is that screen".** `_now_playing_origin` was set whenever a transition landed on NOW_PLAYING while not already there — correct for exactly as long as playback was the only way in. The sleep-timer screen returns to the player, which is also a transition into NOW_PLAYING, so it recorded `SLEEP_TIMER` as the place Back should go: Back then bounced between the player and the duration grid with **no way out to the episode list**, and every list screen behind it was unreachable. The general failure is that "next_state is X" conflates *arriving* at X with *starting something* at X; the entering event (`EpisodeSelected`, `SleepTimerRequested`) says which one it is and cannot be confused by a screen that merely returns there. Worth noting the scripted-touch harness caught this and reading the diff did not — the bug is in the *interaction* between two correct-looking rules.
- **A tap target may be larger than its ink, and on a 296x128 panel it usually should be.** The sleep badge draws inside the 16px top strip, which is well under the 35px finger-sized standard the list rows set — but nothing else up there is tappable, so the *touch* zone reaches down over the title to 72x30 while the ink stays put. No existing layout moves, and the title needs no changes because a title was never a tap target. The same trick already sits in the episode list, whose header doubles as Back. Check what is *actually* competing for a region before shrinking a control to fit its own drawing, and keep the two numbers as separate named constants so nobody later "fixes" the discrepancy.
