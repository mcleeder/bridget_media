...
**Amended 2026-08-23 — the hotspot password is words, not characters.** Twelve random characters (`k7m9qp2rxw4n`) is miserable to transcribe off an e-ink panel: every character has to be read individually, and there is no way to tell you have mistyped one until the join fails with no explanation. It is now two words and four digits — `awoke_mummy_3994` — from `assets/wordlist.txt` (EFF Short Wordlist #1, CC BY 3.0 US, every word 3–5 letters and free of confusable or offensive entries; `yo-yo` dropped, since a hyphen is an awkward keystroke beside the underscores). `_load_words` rejects a list that is short, duplicated or not pure lowercase, so a truncated file fails loudly instead of quietly shrinking the keyspace.

**The entropy cost is real and was priced, not hand-waved: ~34 bits against ~60.** What makes it acceptable *here* is the shape of the attack rather than the number. To use it, someone must be in radio range during the specific window an AP is up — hard-capped at 20 minutes — capture both the handshake and the portal traffic, and only then grind offline; and the per-device SSID is the PBKDF2 salt, so nothing precomputed helps. `_PASSWORD_DIGITS = 6` buys ~6.6 bits for two more keystrokes if that trade ever stops looking right. Underscores also need no `WIFI:` escaping, so the QR payload got simpler rather than more fragile.

Verified by a 24-check harness: list validation (short / uppercase / hyphenated / duplicated / missing / comment-only), format and WPA2's 8–63 bounds over 4000 draws, distinct words, digit coverage in every position, the widest possible password not clipping on the panel, the QR still rendering at version 3, and the ensure/load round trip.
